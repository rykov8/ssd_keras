...
class BaseGTUtility(object):
    ...
    def plot_gt(self, boxes, show_labels=True):
        ...
        for box in boxes:
            label = np.nonzero(box[-num_classes:])[0][0]
            color = self.colors[label]
            is_polygon = len(box) - num_classes > 4
            if is_polygon:
                xy = box[:-num_classes].reshape((-1,2))
            else:
                xmin, ymin, xmax, ymax = box[:4]
                xy = np.array([[xmin, ymin], [xmax, ymin], [xmax, ymax], [xmin, ymax]])
            xy = xy * [h, w]
            ax.add_patch(plt.Polygon(xy, fill=False, edgecolor=color, linewidth=2))
            if show_labels:
        ...
        for i in range(len(gtu.image_names)):
            ...
            for j in range(len(gtu.data[i])):
                ...
                if new_class_idx is not None:
                    ...
                    box = gtu.data[i][j,:-num_old_classes]
                    box = list(box) + class_one_hot
                    boxes.append(box)
            if len(boxes) > 0:
